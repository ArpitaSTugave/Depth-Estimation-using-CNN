...
def load_StereoImages(dirpath='C:/Users/arpit/Documents/Python Scripts/stereodepth_imagepatches'):
    # load training data
    X , X_left,X_right, y =[], [], [], []
    n = 0
...
def image_patches(image1):
    number_labels, na, image_height, image_width = image1.shape
    s = (number_labels,6,image_height,150)
    image_ret = np.zeros(s,dtype=np.float32)
    
    X_train1 = image1[...,0,0:(image1.shape[2]/2),0:(image1.shape[3]/2)]
    X_train2 = image1[...,0,(image1.shape[2]/2):(image1.shape[2]),0:(image1.shape[3]/2)]
    X_train3 = image1[...,0,0:(image1.shape[2]/2),(image1.shape[3]/2):image1.shape[3]]   
    X_train4 = image1[...,0,(image1.shape[2]/2):(image1.shape[2]),(image1.shape[3]/2):image1.shape[3]]    
    s = (number_labels,1,image_height,image_width)    
    X_trainf1 = np.zeros(s,dtype=np.float32)
    X_trainf2 = np.zeros(s,dtype=np.float32)
    X_trainf3 = np.zeros(s,dtype=np.float32)
    X_trainf4 = np.zeros(s,dtype=np.float32)
    for num in range(0,number_labels):
        dummy = X_train1[num,0,...]
        dummy = scipy.ndimage.zoom(dummy, 2, order=3)
        X_trainf1[num,...] = dummy
        dummy = X_train2[num,0,...]
        dummy = scipy.ndimage.zoom(dummy, 2, order=3)
        X_trainf2[num,...] = dummy
        dummy = X_train3[num,0,...]
        dummy = scipy.ndimage.zoom(dummy, 2, order=3)
        X_trainf3[num,...] = dummy
        dummy = X_train4[num,0,...]
        dummy = scipy.ndimage.zoom(dummy, 2, order=3)
        X_trainf4[num,...] = dummy
        
    image_ret = np.concatenate((image1,X_trainf1, X_trainf2,X_trainf3,X_trainf4), axis=1)
    return image_ret
    
X_train_nf,X_left_train,X_right_train, y_train, X_valid_nf,X_left_valid,X_right_valid, y_valid = load_StereoImages()
X_train = image_patches(X_train_nf)
X_valid = image_patches(X_valid_nf)
...
def build_stereo_cnn(input_var=None):
    ...
    network = InputLayer(shape=(None,6, X_train.shape[2], X_train.shape[3]),input_var=input_var,name="input_layer")                                                                                                                             
...
def main(num_epochs=200):
    # Load the dataset                                                                                                                                                                                      
    print("Loading data...")
    X_train_nf,X_left_train,X_right_train, y_train, X_val_nf,X_left_val,X_right_val, y_val = load_StereoImages()
        
    X_train = image_patches(X_train_nf)
    X_val = image_patches(X_val_nf)
